...
export default function Dashboard() {
  ...
  const revenueData = useMemo(() => {
    ...
    const revBookings = safeArray(bookings).filter((b) => {
      if (b.status === STATUS.CANCELLED) return false;
      const ci = safeParseISO(b.check_in, null);
      const co = safeParseISO(b.check_out, null);
      return (ci && isRevInRange(ci)) || (co && isRevInRange(co));
    });

    const totalRevenue = revBookings.reduce((sum, b) => {
      return sum + safeNumber(b.total_amount || b.total_amt);
    }, 0);

    const collected = revBookings.reduce((sum, b) => {
      const total = safeNumber(b.total_amount || b.total_amt);
      const paid = safeNumber(b.total_paid || b.paid_amount);
  ...
  const dashboardData = useMemo(() => {
    // 1. Source Data for Operational Table
    const safeBookingsArray = safeArray(bookings);
    const tableSourceBookings = safeBookingsArray.filter((b) => {
      if (b.status === STATUS.CANCELLED) return false;
      const ci = safeParseISO(b.check_in, null);
      const co = safeParseISO(b.check_out, null);
    ...
    // 2. Source Data for Revenue (Synced with Table Source)
    const revenueSourceBookings = safeBookingsArray.filter((b) => {
      if (b.status === STATUS.CANCELLED) return false;
      const ci = safeParseISO(b.check_in, null);
      const co = safeParseISO(b.check_out, null);
      return (ci && isInRange(ci)) || (co && isInRange(co));
    ...
    const checkInsCount = safeBookingsArray.filter((b) => {
      const date = safeParseISO(b.check_in, null);
      return date && isSameDay(date, today) && b.status !== STATUS.CANCELLED;
    }).length;

    const checkOutsCount = safeBookingsArray.filter((b) => {
      const date = safeParseISO(b.check_out, null);
      return date && isSameDay(date, today) && b.status !== STATUS.CANCELLED;
    }).length;

    const occupiedCount = safeArray(rooms).filter(
      (r) =>
        safeString(r.status).toLowerCase() === "occupied" ||
        safeString(r.status).toLowerCase() === "booked",
    ).length;
    const availableCount = Math.max(0, safeArray(rooms).length - occupiedCount);
    ...
    const filteredList = combinedList.filter((b) => {
      const term = safeString(debouncedSearch).toLowerCase();
      const guestName = safeString(b.guest_name).toLowerCase();
      const bookingId = safeString(b.id).toLowerCase();
      const roomNo = safeString(b.rooms?.[0]?.room_no).toLowerCase();
        ...
        <div className="lg:flex-1 flex flex-col gap-4 lg:min-h-0">
          ...
          <div className="order-3 lg:order-none grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 px-0 mb-4">
            <StatCard
              ...
              value={dashboardData.checkInsCount}
              icon={<Users className="w-5 h-5" />}
              color="blue"
            />
            <StatCard
              title="Check-outs Today"
              value={dashboardData.checkOutsCount}
            ...
            <div className="lg:relative order-2 lg:order-none flex flex-col">
              <div className="lg:flex-1 flex flex-col gap-4 p-0 lg:min-h-0">
                <div className="bg-white/90 backdrop-blur-sm border border-gray-100 shadow-[0_6px_20px_rgba(0,0,0,0.06)] rounded-2xl px-4 py-4 sm:px-5 sm:py-5 flex justify-between items-center ring-1 ring-black/5 min-h-[56px]">
                  ...
                  <button
                    onClick={() => navigate(`/bookings?view=${view}`)}
                    className="shrink-0 text-[10px] sm:text-[11px] text-blue-600 font-bold uppercase tracking-wider hover:underline ml-2"
                  >
                    View All
                  </button>
                </div>

                <ContentCard className="flex flex-col h-[500px] sm:h-[560px] lg:h-[600px] lg:hover:-translate-y-0.5 lg:hover:shadow-[0_12px_32px_rgba(0,0,0,0.10)] transition-all">
                  <div className="flex-1 flex flex-col min-h-0 overflow-hidden text-gray-900 pb-2">
                    <div className="px-4 py-4 sm:p-5 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      <div className="flex items-center justify-between w-full gap-2">
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="w-8 h-8 flex items-center justify-center bg-blue-50 rounded-lg shrink-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                              <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.6)] shrink-0"></span>
                              <p className="text-[9px] text-green-600 font-bold uppercase tracking-widest leading-none truncate">
                                Live
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="flex bg-gray-100 p-1 rounded-xl shrink-0">
                          {["today", "week", "month"].map((v) => (
                            <button
                              ...
                              className={`px-2 sm:px-3 py-1 text-[9px] uppercase tracking-wider rounded-md font-bold transition-all ${
                                view === v
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                      ...
                      >
                        {dashboardData.filteredList.length > 0 ? (
                          <div className="overflow-x-auto w-full">
                            <table className="w-full text-left border-separate border-spacing-0">
                              <thead className="sticky top-0 z-10 bg-gray-50/95 backdrop-blur-sm text-[10px] font-bold text-gray-600 uppercase tracking-widest border-b border-gray-100">
                                <tr>
                                  <th className="pl-8 pr-6 py-4 leading-none text-gray-600">
                                    Guest & Details
                                  </th>
                                  <th className="px-6 py-4 leading-none text-gray-600">
                                    Room
                                  </th>
                                  <th className="px-6 py-4 block sm:table-cell leading-none text-gray-600">
                                    Arrival / Departure
                                  </th>
                                  <th className="px-6 py-4 hidden md:table-cell leading-none text-gray-600">
                                    Status
                                  </th>
                                  <th className="px-6 py-4 text-right leading-none text-gray-600">
                                    Actions
                                  </th>
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-gray-50">
                                {dashboardData.filteredList.map(
                                  (booking, index) => (
                                    <tr
                                    ...
                                    >
                                      <td className="pl-8 pr-6 py-4">
                                        <div className="flex items-center gap-2 mb-1">
                                          <p className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors text-sm tracking-tight leading-relaxed">
                                            {booking.guest_name}
                                          </p>
                                          <div className="flex gap-1">
                                            {booking.opType === "both" ? (
                                              <>
                                                <span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter bg-blue-50 text-blue-600">
                                                  Arrival
                                                </span>
                                                <span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter bg-orange-50 text-orange-600">
                                                  Departure
                                                </span>
                                              </>
                                            ) : (
                                      ...
                                      <td className="px-6 py-4 block sm:table-cell">
                                        <div className="text-[10px] font-bold space-y-1">
                                          <div className="flex items-center gap-2 text-gray-700">
                                            <ArrowRight
                                              className={`w-2.5 h-2.5 ${booking.opType === "check-in" ? "text-blue-500" : "text-gray-300"}`}
                                            />
                                            <span
                                              className={
                                                booking.opType === "check-in"
                                            ...
                                            >
                                              {formatDateSafe(booking.check_in)}
                                            </span>
                                          </div>
                                          <p className="flex items-center gap-2">
                                            <LogOut
                                              className={`w-2.5 h-2.5 ${booking.opType === "check-out" ? "text-orange-500" : "text-gray-300"}`}
                                            />
                                            <span
                                              className={
                                                booking.opType === "check-out"
                                                  ? "text-gray-900"
                                                  : "text-gray-400 font-medium"
                                              }
                                            >
                          ...
                          <div className="flex-1 flex items-start justify-center pt-10 sm:pt-12">
                      ...
                      <div className="shrink-0 border-t border-gray-100">
                        <Pagination
                          ...
                          totalPages={
                            Math.ceil(
                              dashboardData.totalCount / itemsPerPage,
                            ) || 1
                          }
              ...
              <ContentCard className="flex flex-col flex-1 order-5 lg:hover:-translate-y-0.5 lg:hover:shadow-[0_12px_32px_rgba(0,0,0,0.10)] transition-all">
                <div className="px-4 py-4 sm:px-5 sm:py-5 border-b border-gray-50 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-base font-semibold text-gray-900 leading-none truncate capitalize">
                        {view} Summary
                      </h2>
                      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1.5 leading-none">
                        Quick Stats
                      </p>
                    </div>
                  </div>
              ...
              <ContentCard className="p-0 order-1 lg:hover:-translate-y-0.5 lg:hover:shadow-[0_12px_32px_rgba(0,0,0,0.10)] transition-all flex flex-col flex-1">
                <div className="px-4 py-4 sm:px-5 sm:py-5 border-b border-gray-50 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-base font-semibold text-gray-900 leading-none truncate">
                        Revenue
                      </h2>
                      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1.5 leading-none">
                        Overview
                ...
                <div className="flex-1 px-4 py-4 flex flex-col justify-center items-center">
                  ...
                  <div className="mt-3 w-full grid grid-cols-2 gap-2">
                    ...
                    <div className="bg-emerald-50 rounded-xl p-2.5 text-center border border-emerald-100/50 group hover:bg-emerald-100/50 transition-colors">
                      <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-widest mb-1.5 leading-none">
                        Collected
                      </p>
                      <p className="text-sm font-semibold text-emerald-700 tabular-nums leading-none">
                        ₹{" "}
                        {revenueData.collected.toLocaleString("en-IN", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </p>
                    ...
                    <div className="bg-orange-50 rounded-xl p-2.5 text-center border border-orange-100/50 group hover:bg-orange-100/50 transition-colors">
                      <p className="text-[10px] text-orange-600 font-bold uppercase tracking-widest mb-1.5 leading-none">
                        Pending
                      </p>
                      <p className="text-sm font-semibold text-orange-700 tabular-nums leading-none">
                        ₹{" "}
                        {revenueData.pending.toLocaleString("en-IN", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </p>
              ...
              <ContentCard className="flex flex-col flex-1 order-2 lg:hover:-translate-y-0.5 lg:hover:shadow-[0_12px_32px_rgba(0,0,0,0.10)] transition-all">
                <div className="px-4 py-4 sm:px-5 sm:py-5 border-b border-gray-50 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-base sm:text-lg font-semibold text-gray-900 tracking-tight leading-none truncate">
                        Room Availability
                      </h2>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.6)] shrink-0"></span>
                        <p className="text-[10px] sm:text-[11px] text-green-600 font-bold uppercase tracking-widest leading-none truncate">
                          Live Status
                        </p>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="shrink-0 text-[10px] sm:text-[11px] font-bold text-blue-600 uppercase tracking-widest hover:text-blue-700 transition-colors flex items-center gap-0.5 group"
                  >
                    Manage{" "}
                ...
                <div className="flex-1 px-4 py-5 sm:px-5 sm:py-6 flex flex-col justify-center space-y-4">
                  <RoomStatusRow
                    label="Available Now"
                    count={dashboardData.availableCount}
                    color="green"
                  />
                  <RoomStatusRow
                    label="Occupied"
                    count={dashboardData.occupiedCount}
                    color="purple"
                  />
                </div>
              </ContentCard>
  ...
  return (
    <div className="bg-white border border-gray-200 p-3 sm:p-4 rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.08)] lg:hover:-translate-y-1 lg:hover:shadow-[0_12px_32px_rgba(0,0,0,0.10)] transition-all flex items-center justify-between group active:scale-[0.98]">
      <div className="space-y-1">
        ...
        <p className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tighter tabular-nums">
          {value}
        </p>
      </div>
      <div
        className={`w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-lg border ${colors[color]} lg:group-hover:scale-110 lg:transition-transform`}
      >
        {icon}
      </div>
    </div>
...
function StatusBadge({ status }) {
  const styles = {
    ...
    [STATUS.CHECKED_OUT]:
      "bg-gray-50 text-gray-500 border-gray-200 shadow-inner",
    [STATUS.CANCELLED]: "bg-red-50 text-red-700 border-red-200",
  };

  return (
    <span
      className={`px-2 py-0.5 rounded-md text-[9px] uppercase font-bold border tracking-wider ${styles[status] || styles[STATUS.PENDING]}`}
    >
      {status}
    </span>
  );
...
function ActionButton({ label, icon, onClick, color }) {
  const bg =
    color === "blue"
      ? "bg-blue-600 text-white hover:bg-blue-700 shadow-sm text-white"
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="py-20 text-center bg-white rounded-3xl border border-red-50">
      ...
      <p className="text-gray-400 text-sm mb-6 max-w-xs mx-auto font-medium">
        {message}
      </p>
      <button
        onClick={() => onRetry()}
        className="px-8 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 shadow-lg shadow-red-200 transition-all inline-flex items-center gap-2 text-sm uppercase"
      >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="bg-white/90 p-5 rounded-2xl border border-gray-100 shadow-md flex justify-between items-center h-[108px]"
              >
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div className="space-y-3">
                    {[...Array(2)].map((_, l) => (
                      <div
                        key={l}
                        className="h-8 w-full bg-gray-50/50 rounded-lg animate-shimmer"
                      />
                    ))}
                  </div>
